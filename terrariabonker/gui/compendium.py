...
from PyQt6.QtCore import QProcess, QSize, QSortFilterProxyModel, Qt
...
from PyQt6.QtWidgets import (QAbstractItemView, QComboBox, QDialog, QHBoxLayout, QHeaderView,
                             QLabel, QLineEdit, QPushButton, QTreeView, QVBoxLayout, QWidget)
...
class EntryDialog(QDialog):
    ...
    def __init__(self, parent, entry: dict, icon, on_give):
        super().__init__(parent)
        name = entry.get("name", "?")
        ...
        shown = [("Damage", stats.get("damage")), ("Defense", stats.get("defense")),
                 ("Rarity", stats.get("rare")), ("Use time", stats.get("use_time")),
                 ("Pickaxe", stats.get("pick"))]
...
class CompendiumTab(QWidget):
    ...
    def __init__(self, parent, fetch, on_give, icon_for, log):
        super().__init__(parent)
        self._fetch = fetch
    ...
    def _open_entry(self, proxy_index) -> None:
        ...
        icon = item.icon() if entry.get("kind") != "NPC" else None
        self._opening = True
        try:
            EntryDialog(self, entry, icon, self._give).exec()
        finally:
            self._opening = False
